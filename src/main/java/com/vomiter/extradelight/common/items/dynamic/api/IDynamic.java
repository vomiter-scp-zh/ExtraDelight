        ...
        for (ItemStack ingredient : items) {
            if(ingredient.getItem() instanceof IDynamic){
                for (ItemStack dynamicIngredient : DataComponents.getDynamicIngredients(ingredient)) {
                    var ingRc = ForgeRegistries.ITEMS.getKey(dynamicIngredient.getItem());
                    if(ingRc == null) continue;
                    i.add(ExtraDelight.modLoc(prefix + "dynamic_jam/" + ingRc.getPath()));
                }
            }
            else {
                var ingRc = ForgeRegistries.ITEMS.getKey(ingredient.getItem());
                if(ingRc == null) continue;
                i.add(ExtraDelight.modLoc(prefix + ingRc.getPath()));
            }
        }
        if(i.size() <= 1){
